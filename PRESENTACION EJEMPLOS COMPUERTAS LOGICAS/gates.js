...
function buildGate(g, gi) {
  ...
  sec.innerHTML = `
    ...
    <header class="g-head">
      <div class="g-title">
        <div><span class="chip" data-a>${num} · ${g.kind}</span><h2 class="g-name"><span class="split">${g.name}</span></h2></div>
        <div class="g-story" data-a><h3>${g.title}</h3><p>${g.lead}</p></div>
      </div>
      <div class="g-card glass corners" data-a><div class="sym-slot"></div><div><code>${g.expr}</code><small>Ladder: ${g.read}</small></div></div>
    </header>
    <div class="g-body${gi % 2 ? " mirror" : ""}">
      ...
      <div class="logic glass" data-a="scale">
        <div class="logic-top">
          <div class="tabs" role="tablist" data-tab="0"><span class="ind"></span><button role="tab" aria-selected="true" data-tab="0">Tabla + Ladder</button><button role="tab" aria-selected="false" data-tab="1">Compuertas</button><button role="tab" aria-selected="false" data-tab="2">Cronograma</button></div>
        ...
        <div class="view v0">
          <div class="tt-wrap">
            ...
            <p class="tip">${manual ? "Toca una fila o un contacto." : "Entradas automáticas."}</p>
          </div>
          <div class="ladder-wrap"><p class="lbl">Ladder · ${g.read}</p><div class="ladder-slot"></div></div>
        </div>
        <div class="view vg" hidden>
        ...
        <div class="view vc" hidden><div class="chrono-slot" style="flex:1;min-height:0;display:flex"></div><p class="tip">Últimos 12 s. La última fila es la salida ${g.out}.</p></div>
      </div>
    </div>
  ...
  const sc = {
    ...
    tick(dt) {
      sim.tick?.(sc, dt);
      if (sc.tab === 2 && (sc.chronoT = (sc.chronoT || 0) + dt) > 1 / 15) {
        sc.chronoT = 0;
  ...
  $$(".tabs button", sec).forEach(bt => bt.addEventListener("click", () => {
    ...
    if (sc.tab === 2) $(".chrono-slot", sec).innerHTML = chronoSVG([...keys, g.out], sc.hist);
    sfx.click();
  }));
